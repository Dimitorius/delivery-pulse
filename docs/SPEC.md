...
Статус: **утверждено** Дмитрием 25.09.2026 (все раунды grilling закрыты).

**Прогресс:** этап 0 **закрыт** 25.09.2026 — сайт: https://dimitorius.github.io/delivery-pulse/ · этап 1 **закрыт** 25.09.2026, ревью Дмитрия принято и внесено ([stage-1.md](stage-1.md)) · этап 2 **закрыт** 25.09.2026 ([stage-2.md](stage-2.md), решения на просмотр) · следующий — этап 3.
...
- Предзаполненная история — 3 PI (~7 месяцев) + живой хвост в ускоренном времени (пауза, 1× = 1 сек → 1 рабочий час, 10×, 100×). *(ревью 25.09)*
- **PI = 4 итерации + 1 IP-итерация (Innovation & Planning) = 10 недель.** Commitment PI ≈ 80% ёмкости; есть uncommitted (stretch) objectives. *(ревью 25.09)*
...
- **Базовая линия — elite** *(ревью 25.09)*: прогноз PI (Monte Carlo) на конец истории ~85–90%; без сценариев почти все плитки зелёные, допустимо 1–2 жёлтых. Падение до «под угрозой» — только через сценарии (Inject scenario).
...
- Правила статуса *(ревью 25.09)*:
  - **Low confidence:** при выборке n < 10 значение помечается «low confidence» и не окрашивается.
  - **Коридорные цели:** Say/Do — коридор 80–90%, не «≥ 80%»; устойчиво > 95% (все 3 последних спринта команды) → пометка «possible sandbagging».
  - **Гистерезис:** статус меняется, только если новое значение держится за порогом несколько обновлений подряд (сейчас 3), — чтобы 60% ↔ 59% не мигало.
  - **Бенчмарки по типам:** research (DORA — подпись «DORA 2024 performance clusters» + примечание, что в DORA 2025 кластеры заменены 7 архетипами) · disputed (показываются все позиции, без одной «правильной» цифры; напр. flow efficiency: Kanban University vs данные ASOS) · team goal (Say/Do, PR pickup, main green, unplanned, blocked, overdue deps — «наша цель/порог», не бенчмарк; вендорские данные — только с подписью «vendor dataset») · method · by design.
...
- **Pulse**: шапка (уровень, часы симуляции, Inject scenario, счётчик сигналов, Framework lens) → строка «успеваем?» (MC P50/P85 vs дедлайн PI, вероятность) → 3 колонки Lagging/Current/Leading по 5–7 плиток → справа: **Signals** (только XmR-сигналы и нарушения целей; счётчик в шапке = их число) · **Watch items** (отдельные задачи старше SLE, просроченные зависимости) · лента событий *(ревью 25.09)* → мини-граф зависимостей. Net Flow подписан «arrived − finished». Клик по плитке → страница метрики; по сигналу → Diagnose.
...
## 11. Этапы
0. Репо + деплой + пустая страница по ссылке. ✅ закрыт 25.09.2026 — https://dimitorius.github.io/delivery-pulse/
1. Симулятор, модель событий, ~15 метрик, Pulse. ✅ закрыт 25.09.2026 — 17 метрик, см. [stage-1.md](stage-1.md)
2. Ядро 50, все табы, страница метрики. → + Tour = первая показываемая версия (~2–3 дня). ✅ закрыт 25.09.2026 (ядро 50 + 4 SYNTHETIC, все табы, страница метрики, Framework lens) — [stage-2.md](stage-2.md); Tour — вместе с этапом 3 (нужен Inject scenario)
